...
def update_customer():
    ...
    try:
        ...
        logging.debug(f"Loyverse response: {loyverse_response.json()}")
    except requests.exceptions.RequestException as e:
    ...
    try:
        ...
        ''', (display_name, email, home_country, birthdate, gender, customer_code))

        if cursor.rowcount == 0:
            logging.debug("No rows updated. Trying to insert new row.")
            cursor.execute('''
                INSERT INTO loyalty_accounts (display_name, email, home_country, birthdate, gender, customer_code)
                VALUES (%s, %s, %s, %s, %s, %s)
            ''', (display_name, email, home_country, birthdate, gender, customer_code))
